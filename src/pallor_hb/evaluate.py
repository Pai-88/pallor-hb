...
def roc_comparison_plot(curves: dict, path: str, cutoff: float) -> None:
    """Overlay ROC curves for several configurations on one axis.
    ...
    for (label, (y_true, y_pred)), style in zip(curves.items(), ["--", "-", ":"]):
        y_bin = (np.asarray(y_true, dtype=float) < cutoff).astype(int)
        # Anemia is low Hb, so the score is the negated prediction.
        fpr, tpr, _ = roc_curve(y_bin, -np.asarray(y_pred, dtype=float))
        ax.plot(fpr, tpr, style, lw=1.8, label=label)
    ax.plot([0, 1], [0, 1], color="k", lw=0.8, alpha=0.5)
    ax.set_xlabel("False positive rate")
    ax.set_ylabel("True positive rate (sensitivity)")
    ax.set_title(f"Anemia screening ROC (Hb < {cutoff:g} g/dL)")
